...
{
    public class GeodeProcessMenu : MenuWithInventory
    {
        ...
        private int _geodeAnimationTimer;
        ...
        public GeodeProcessMenu(IContentHelper content)
            : base(null, true, true, 12, 132)
        {
            ...
            _clint = new AnimatedSprite(_content.GetActualAssetKey("assets/Empty.png"), 8, 32, 48);
        ...
        {
            if (base.readyToClose() && _geodeAnimationTimer <= 0)
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = false)
        {
            ...
            if (!GeodeSpot.containsPoint(x, y))
                return;
            if (heldItem != null && (heldItem.Name.Contains("Geode") || heldItem.ParentSheetIndex == 275) &&
                _geodeAnimationTimer <= 0)
            {
                    ...
                    Game1.player.freeSpotsInInventory() == 1 && heldItem.Stack == 1)
                {
                    ...
                    if (heldItem.Stack <= 0)
                        heldItem = null;
                    _geodeAnimationTimer = 200;
                    Game1.playSound("stoneStep");
                    _clint.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
                    {
                        new FarmerSprite.AnimationFrame(8, 0),
                        new FarmerSprite.AnimationFrame(9, 0),
                        new FarmerSprite.AnimationFrame(10, 0),
                        new FarmerSprite.AnimationFrame(11, 0),
                        new FarmerSprite.AnimationFrame(12, 0),
                        new FarmerSprite.AnimationFrame(8, 0)
                    });
                    _clint.loop = false;
                }
        ...
        {
            base.update(time);
            for (var index = _fluffSprites.Count - 1; index >= 0; --index)
                if (_fluffSprites[index].update(time))
                    _fluffSprites.RemoveAt(index);

            if (_alertTimer > 0)
                _alertTimer -= time.ElapsedGameTime.Milliseconds;
            if (_geodeAnimationTimer <= 0)
                return;
            _geodeAnimationTimer -= time.ElapsedGameTime.Milliseconds;
            if (_geodeAnimationTimer <= 0)
            {
            ...
            else
            {
                ...
                if (_clint.currentFrame == 11 && currentFrame != 11)
                {
                    ...
                    if (GeodeSpot.item != null)
                    {
                        ...
                        _geodeDestructionAnimation = new TemporaryAnimatedSprite("TileSheets\\animations",
                            ...
                            new Vector2(GeodeSpot.bounds.X + 285 - 32, GeodeSpot.bounds.Y + 150 - 32), false, false);
                        if (GeodeSpot.item != null && GeodeSpot.item.ParentSheetIndex == 275)
                        {
                            ...
                            for (var index = 0; index < 6; ++index)
                            {
                                _fluffSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors",
                                    ...
                                    new Vector2(GeodeSpot.bounds.X + 285 - 32 + Game1.random.Next(21),
                                        GeodeSpot.bounds.Y + 150 - 16), false, 1f / 500f,
                ...
                if (_geodeDestructionAnimation != null &&
                    !(_geodeDestructionAnimation.id.Equals(777) &&
                      _geodeDestructionAnimation.currentParentTileIndex < 7 ||
                      _geodeDestructionAnimation.id.Equals(777) &&
                      _geodeDestructionAnimation.currentParentTileIndex < 5))
                {
                    ...
                    else
                    {
                        ...
                        if ((_geodeDestructionAnimation.currentParentTileIndex == 7 ||
                             _geodeDestructionAnimation.id.Equals(777) &&
                             _geodeDestructionAnimation.currentParentTileIndex == 5) && _geodeTreasure.Price > 75)
                        {
                            _sparkle = new TemporaryAnimatedSprite("TileSheets\\animations",
                                ...
                                new Vector2(GeodeSpot.bounds.X + 285 - 32,
                                    GeodeSpot.bounds.Y + 150 + _yPositionOfGem - 32), false, false);
        ...
        public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
        {
            ...
            inventory = new InventoryMenu(xPositionOnScreen + spaceToClearSideBorder + borderWidth / 2 + 12, yPosition,
                false, null, inventory.highlightMethod);
        }
        ...
        public override void draw(SpriteBatch b)
        {
            ...
            b.Draw(_content.Load<Texture2D>("assets/bgpatch.png"), new Vector2(GeodeSpot.bounds.X, GeodeSpot.bounds.Y),
                new Rectangle(0, 0, 140, 78), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None,
                0.87f);
            if (GeodeSpot.item != null)
            {
                if (_geodeDestructionAnimation == null)
